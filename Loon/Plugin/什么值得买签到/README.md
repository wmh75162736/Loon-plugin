...
- `显示立即签到`: 默认关闭。需要手动立刻出结果时打开，然后运行脚本资源里的 `什么值得买_立即签到`，该资源不走随机延迟。
- `显示账号管理`: 默认关闭。开启后可在脚本资源运行 `什么值得买_账号管理`。
- `账号管理操作`: `list` 为查看账号列表；`delete` 为删除 `删除账号序号` 所对应的账号。
- `删除账号序号`: 从 1 开始计数，仅在账号管理操作设为 `delete` 时生效。
...
1. 在插件“设置”里打开 `显示立即签到`。
2. 在脚本资源里运行 `什么值得买_立即签到`。
3. 该手动资源会跳过随机延迟并强制发送结果通知，即使通知模式设置为 `never`。
4. 成功执行后会标记当天已签到，避免当天自动任务重复签到。
5. 用完可关闭 `显示立即签到`。

## 已保存账号管理

1. 在插件“设置”中打开 `显示账号管理`。
2. 将 `账号管理操作` 保持为 `list`，在脚本资源运行 `什么值得买_账号管理`，通知会显示账号序号、脱敏会话标识和 Cookie 更新时间。
3. 需要删除账号时，将 `账号管理操作` 改为 `delete`，在 `删除账号序号` 填入目标账号序号，再运行一次 `什么值得买_账号管理`。
4. 删除成功后，建议将 `账号管理操作` 改回 `list`，避免下次运行时重复删除同一序号。
5. 管理完成后可关闭 `显示账号管理`。完整 Cookie 不会展示在通知中。
