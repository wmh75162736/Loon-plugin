...
1. 在插件“设置”中打开 `显示账号管理`。
2. 在脚本资源运行 `什么值得买_查看账号`，通知会显示账号序号、脱敏会话标识和 Cookie 更新时间。
3. 需要删除账号时，直接运行同序号的资源，例如账号列表中的“账号1”对应 `什么值得买_删除账号1`。
4. 删除后，原账号2会自动变为新的账号1；如需继续删除，请先重新运行 `什么值得买_查看账号` 确认序号。
